...
let _silkWasmPromise: Promise<SilkWasmModule | null> | null = null;

/** Lazy-load the silk-wasm module (singleton cache; returns null on failure). */
export function loadSilkWasm(): Promise<SilkWasmModule | null> {
  if (_silkWasmPromise) {
    return _silkWasmPromise;
  }
  _silkWasmPromise = (async () => {
    try {
      const mod = (await import("silk-wasm")) as unknown as SilkWasmModule;
      return mod;
    } catch {
      return null;
    }
...
const NOOP_LOGGER: Required<AudioLogger> = {
  log: () => {},
  warn: () => {},
  error: () => {},
};

function resolveLogger(logger?: AudioLogger): Required<AudioLogger> {
  return {
...
/**
 * Minimal typed surface of `mpg123-decoder` we depend on. Defined locally
 * so the SDK builds cleanly without having `mpg123-decoder` installed.
 */
interface Mpg123DecoderModule {
  MPEGDecoder: new () => {
...
export async function wasmDecodeMp3ToPCM(
  buf: Buffer,
  targetRate: number,
  logger?: AudioLogger,
): Promise<Buffer | null> {
  const log = resolveLogger(logger);
  try {
    const mod = (await import("mpg123-decoder")) as unknown as Mpg123DecoderModule;
