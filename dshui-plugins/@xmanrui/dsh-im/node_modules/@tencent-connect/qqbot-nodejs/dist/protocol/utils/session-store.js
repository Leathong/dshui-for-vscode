...
export class FileSessionStore {
    ...
    load(accountId, expectedAppId) {
        try {
            ...
            if (now - state.savedAt > this.options.expireMs) {
                this.options.logger.log(`[session-store] Session expired for ${accountId}, age: ${Math.round((now - state.savedAt) / 1000)}s`);
                try {
            ...
            if (expectedAppId && state.appId && state.appId !== expectedAppId) {
                this.options.logger.log(`[session-store] appId mismatch for ${accountId}: saved=${state.appId}, current=${expectedAppId}. Discarding stale session.`);
                try {
                    fs.unlinkSync(filePath);
                }
                catch {
            ...
            this.options.logger.log(`[session-store] Loaded session for ${accountId}: sessionId=${state.sessionId}, lastSeq=${state.lastSeq}, appId=${state.appId ?? "unknown"}, age=${Math.round((now - state.savedAt) / 1000)}s`);
            return state;
        }
        catch (err) {
            this.options.logger.error(`[session-store] Failed to load session for ${accountId}: ${formatErrorMessage(err)}`);
            return null;
        }
    }
    ...
    clear(accountId) {
        ...
        try {
            ...
            this.options.logger.error(`[session-store] Failed to clear session for ${accountId}: ${formatErrorMessage(err)}`);
        }
    }
    /** Update only lastSeq on the persisted session. */
    updateLastSeq(accountId, lastSeq) {
    ...
    // ============ Internal ============
    encodeAccountIdForFileName(accountId) {
        return Buffer.from(accountId, "utf8").toString("base64url");
    }
    getLegacySessionPath(accountId) {
    ...
    getCandidateSessionPaths(accountId) {
    ...
    doSaveSession(state) {
        ...
        try {
            ...
            this.options.logger.log(`[session-store] Saved session for ${state.accountId}: sessionId=${state.sessionId}, lastSeq=${state.lastSeq}`);
        }
        catch (err) {
            this.options.logger.error(`[session-store] Failed to save session for ${state.accountId}: ${formatErrorMessage(err)}`);
        }
    }
}
