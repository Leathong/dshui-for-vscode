...
function resolveLogger(logger) {
    return {
        log: logger?.log ?? NOOP_LOGGER.log,
...
export function pcmToWav(pcmData, sampleRate, channels = 1, bitsPerSample = 16) {
...
export function stripAmrHeader(buf) {
 ...
 */
export async function audioFileToSilkBase64(filePath, directUploadFormats, logger) {
    ...
    if (buf.length === 0) {
        log.error(`[audio-convert] file is empty: ${filePath}`);
        return null;
    }
    const ext = lowerExt(filePath);
    ...
    if (uploadFormats.includes(ext)) {
        log.log(`[audio-convert] direct upload (QQ native format): ${ext} (${buf.length} bytes)`);
    ...
    if ([".slk", ".slac"].includes(ext)) {
        ...
        const silk = await loadSilkWasm();
        if (silk?.isSilk(raw)) {
            log.log(`[audio-convert] SILK file, direct use: ${filePath} (${buf.length} bytes)`);
            return buf.toString("base64");
        }
    }
    ...
    if (silkForCheck?.isSilk(rawCheck) || silkForCheck?.isSilk(strippedRaw)) {
        log.log(`[audio-convert] SILK detected by header: ${filePath} (${buf.length} bytes)`);
        return buf.toString("base64");
    }
    ...
    if (ffmpegCmd) {
        try {
            log.log(`[audio-convert] ffmpeg (${ffmpegCmd}): converting ${ext} (${buf.length} bytes) → PCM s16le ${targetRate}Hz`);
            const pcmBuf = await ffmpegToPCM(ffmpegCmd, filePath, targetRate);
            if (pcmBuf.length === 0) {
                log.error(`[audio-convert] ffmpeg produced empty PCM output`);
                return null;
            }
            const { silkBuffer } = await pcmToSilk(pcmBuf, targetRate);
            log.log(`[audio-convert] ffmpeg: ${ext} → SILK done (${silkBuffer.length} bytes)`);
            return silkBuffer.toString("base64");
        }
        catch (err) {
            log.error(`[audio-convert] ffmpeg conversion failed: ${formatErrorMessage(err)}`);
        }
    }
    log.log(`[audio-convert] fallback: trying WASM decoders for ${ext}`);
    if (ext === ".pcm") {
    ...
    if (ext === ".mp3" || ext === ".mpeg") {
        const pcmBuf = await wasmDecodeMp3ToPCM(buf, targetRate, logger);
        if (pcmBuf) {
            const { silkBuffer } = await pcmToSilk(pcmBuf, targetRate);
            log.log(`[audio-convert] WASM: MP3 → SILK done (${silkBuffer.length} bytes)`);
            return silkBuffer.toString("base64");
        }
    }
    ...
    log.error(`[audio-convert] unsupported format: ${ext} (no ffmpeg available). ${installHint}`);
    return null;
}
/**
 ...
 */
export async function waitForFile(filePath, timeoutMs = 30000, pollMs = 500, logger) {
    const log = resolveLogger(logger);
    ...
    while (Date.now() - start < timeoutMs) {
        ...
        try {
            ...
            if (!fileExists) {
                ...
                log.log(`[audio-convert] waitForFile: file appeared (${stat.size} bytes, after ${Date.now() - start}ms): ${path.basename(filePath)}`);
            }
            if (stat.size > 0) {
                if (stat.size === lastSize) {
                    stableCount++;
                    if (stableCount >= 2) {
                        log.log(`[audio-convert] waitForFile: ready (${stat.size} bytes, waited ${Date.now() - start}ms, polls=${pollCount})`);
                        return stat.size;
                    }
                }
                else {
                ...
                if (Date.now() - fileAppearedAt > emptyGiveUpMs) {
                    log.error(`[audio-convert] waitForFile: file still empty after ${emptyGiveUpMs}ms, giving up: ${path.basename(filePath)}`);
                    return 0;
                }
            }
        }
            ...
            if (!fileExists && Date.now() - start > noFileGiveUpMs) {
                log.error(`[audio-convert] waitForFile: file never appeared after ${noFileGiveUpMs}ms, giving up: ${path.basename(filePath)}`);
                return 0;
            }
        }
    ...
    try {
        ...
        if (finalStat.size > 0) {
            log.warn(`[audio-convert] waitForFile: timeout but file has data (${finalStat.size} bytes), using it`);
            return finalStat.size;
        }
        log.error(`[audio-convert] waitForFile: timeout after ${timeoutMs}ms, file exists but empty (0 bytes): ${path.basename(filePath)}`);
    }
    catch {
        log.error(`[audio-convert] waitForFile: timeout after ${timeoutMs}ms, file never appeared: ${path.basename(filePath)}`);
    }
    return 0;
}
...
export function ffmpegToPCM(ffmpegCmd, inputPath, sampleRate = 24000) {
...
export async function wasmDecodeMp3ToPCM(buf, targetRate, logger) {
    const log = resolveLogger(logger);
    try {
        const mod = (await import("mpg123-decoder"));
        const { MPEGDecoder } = mod;
        log.log(`[audio-convert] WASM MP3 decode: size=${buf.length} bytes`);
        ...
        if (decoded.samplesDecoded === 0 || decoded.channelData.length === 0) {
            log.error(`[audio-convert] WASM MP3 decode: no samples (samplesDecoded=${decoded.samplesDecoded})`);
            return null;
        }
        log.log(`[audio-convert] WASM MP3 decode: samples=${decoded.samplesDecoded}, sampleRate=${decoded.sampleRate}, channels=${decoded.channelData.length}`);
        let floatMono;
        ...
        log.error(`[audio-convert] WASM MP3 decode failed: ${formatErrorMessage(err)}`);
        if (err instanceof Error && err.stack) {
            log.error(`[audio-convert] stack: ${err.stack}`);
        }
        return null;
    }
}
/** Parse a standard PCM WAV and extract mono 24 kHz PCM data (fallback without ffmpeg). */
export function parseWavFallback(buf) {
    if (buf.length < 44) {
        return null;
    }
