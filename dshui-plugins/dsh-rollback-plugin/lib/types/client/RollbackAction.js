...
import { IconUndoOutline16 } from "./icons.js";
...
export function RollbackAction(props) {
    ...
    const runExecute = async () => {
        ...
        const remote = await execute(target, request).catch((executeError) => {
 ...
 */
export function TurnRollbackAction(props) {
    const { matched, prepare, execute, openAt, forkAt, t } = props;
    return _jsx(RollbackAction, { target: { turn: matched.turn }, prepare: prepare, execute: execute, openAt: openAt, forkAt: forkAt, t: t });
}
